...
```bash
pip install "velora-ui @ git+https://github.com/gioelecavallo13/Velora.git@v0.1.0"
```
...
|---|---|---|
| **v0.1.0** | in release | Layout, form 7 tipi base, tabella + paginazione, 4 link, feedback, showcase, Docker dev+prod |
| v0.2.0 | pianificato | Header multi-menu/apps-menu/notifications, breadcrumb, tooltip, submenu, dropdown, dialog, copy/toggle/settings link, progress bar (3 varianti) |
...
Versione corrente: vedi [`pyproject.toml`](pyproject.toml). Le release alpha (`v0.1.0-alpha.x`) hanno chiuso le milestone M1 → M5; la release candidate `v0.1.0-rc.1` ha chiuso M6 (showcase + visual regression). La v0.1.0 stabile chiude M7 (questa release). Il progetto e` distribuito via Git privato, non e` ancora pubblicato su PyPI.
